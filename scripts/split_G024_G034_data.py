...
def main(indir="."):
    ...
    for source in ["G024", "G034"]:
        data = {}
        for transition in transitions:
            with open(os.path.join(indir, f"{source}_{transition}.pkl"), "rb") as f:
        ...
        print(f"{source} 1612 MHz Continuum rms: {rms:.3f} Jy/beam")
        ...
        outdir = os.path.join(indir, source)
        ...
        for idx, coord in enumerate(zip(*np.where(~mask))):
            ...
            for transition in transitions:
                ...
                datum[f"absorption_{transition}"] = data[transition]["absorption"][
                    :, *coord
                ]
                datum[f"absorption_rms_{transition}"] = data[transition][
                    "absorption_rms"
                ][*coord]
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        print("python split_G024_G034_data.py <indir>")
    else:
